...
final class AdminGenerator
{
    ...
    public function generate(string $entityName): array
    {
        ...
        if (!is_file($searchComponentPath)) {
            $this->filesystem->dumpFile(
                $searchComponentPath,
                $this->renderSearchComponent($componentName.'Search'),
            );
        }
        $this->filesystem->dumpFile(
            $templatePath,
...
export default function {$componentName}({ apiUrl }: {$componentName}Props) {
    return (
        <EntityCrudGrid
            ...
            idField={$idJson}
            fields={fields}
            SearchComponent={{$componentName}Search}
...
TSX;
    ...
    private function renderSearchComponent(string $searchComponentName): string
    {
        return <<<TSX
